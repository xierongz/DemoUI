...
static S_ButtonCallback sButtonCallbackTab[] = {
    ID_POINTER_sys_back, onButtonClick_sys_back,
};
/***************/
...
void pointerActivity::onCreate() {
	Activity::onCreate();
    mDiagram1Ptr = (ZKDiagram*)findControlByID(ID_POINTER_Diagram1);
    msys_backPtr = (ZKButton*)findControlByID(ID_POINTER_sys_back);
...
void pointerActivity::onVideoPlayerMessage(ZKVideoView *pVideoView, int msg) {
...
void pointerActivity::videoLoopPlayback(ZKVideoView *pVideoView, int msg, int callbackTabIndex) {
...
void pointerActivity::startVideoLoopPlayback() {
    int tablen = sizeof(SVideoViewCallbackTab) / sizeof(S_VideoViewCallback);
    for (int i = 0; i < tablen; ++i) {
    	if (SVideoViewCallbackTab[i].loop) {
...
void pointerActivity::stopVideoLoopPlayback() {
    int tablen = sizeof(SVideoViewCallbackTab) / sizeof(S_VideoViewCallback);
    for (int i = 0; i < tablen; ++i) {
    	if (SVideoViewCallbackTab[i].loop) {
    		ZKVideoView* videoView = (ZKVideoView*)findControlByID(SVideoViewCallbackTab[i].id);
...
bool pointerActivity::parseVideoFileList(const char *pFileListPath, std::vector<string>& mediaFileList) {
	mediaFileList.clear();
...
int pointerActivity::removeCharFromString(string& nString, char c) {
