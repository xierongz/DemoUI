...
static S_VideoViewCallback SVideoViewCallbackTab[] = {
    ID_SCREENSAVER_Videoview1, true, 5, NULL,
};
...
void screensaver::onCreate() {
	BaseApp::onCreate();
    mTextview2Ptr = (ZKTextView*)findControlByID(ID_SCREENSAVER_Textview2);
    mVideoview1Ptr = (ZKVideoView*)findControlByID(ID_SCREENSAVER_Videoview1);if(mVideoview1Ptr!= NULL){mVideoview1Ptr->setVideoPlayerMessageListener(this);}
    mTextview1Ptr = (ZKTextView*)findControlByID(ID_SCREENSAVER_Textview1);
    mWindow1Ptr = (ZKWindow*)findControlByID(ID_SCREENSAVER_Window1);
        // 注册监听全局触摸
	onUI_init();
...
int screensaver::getListItemCount(const ZKListView *pListView) const{
    ...
    for (int i = 0; i < tablen; ++i) {
        if (SListViewFunctionsCallbackTab[i].id == pListView->getID()) {
            SListViewFunctionsCallbackTab[i].getListItemCountCallback(pListView);
            break;
        }
    }
    return 0;
...
void screensaver::videoLoopPlayback(ZKVideoView *pVideoView, int msg, int callbackTabIndex) {
