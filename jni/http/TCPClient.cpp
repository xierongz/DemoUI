...
std::string TCPClient::Sendnix(std::string hostname, std::string portNumber, std::string message) {
  //TCP client
   ...
   // LOGD("getaddrinfo:%s\r\n",hostname.c_str());
    status = getaddrinfo(hostname.c_str(), portNumber.c_str(), &host_info, &host_info_list);
    //LOGD("getaddrinfo status:%d,%d\r\n",status,host_info_list);
   ...
   // LOGD("connect socket...\r\n");
    ...
    char* incomming_data_buffer = new char[1024*1024];
    bytes_recieved = recv(socketfd, incomming_data_buffer,1024*1024 - 1, 0);
    ...
    std::string instr(incomming_data_buffer);
    delete incomming_data_buffer;
    return instr;
}
#endif
