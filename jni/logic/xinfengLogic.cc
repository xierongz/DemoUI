...
static void UpdateUI_CO2(){
	...
	if(mProtocolData.co2 > 3000){
		mButtonCO2StatusPtr->setText("极差");
...
static bool onButtonClick_btn_auto(ZKButton *pButton) {
	...
	if(getProtocolData().childLock){
		...
		strpasswd.clear();
		mTextPassPtr->setText("已锁定，请输入童锁密码");
		return false;
	}
